...
pub unsafe extern "C" fn esp_loader_source() -> CSlice {
    CSlice::from_slice(Refind::esp_loader_source().map(str::as_bytes))
}
...
pub unsafe extern "C" fn set_one_shot(request: *const CBootPluginRequest, err_out: *mut ErrorKind) -> i32 {
    ...
    let result = entry_name_from_request(unsafe { &*request })
        .and_then(|entry_name| Refind::new().and_then(|mut refind| refind.set_one_shot(&entry_name)));
...
pub unsafe extern "C" fn confirm_boot(request: *const CBootPluginRequest, err_out: *mut ErrorKind) -> i32 {
    ...
    let result = entry_name_from_request(unsafe { &*request })
        .and_then(|entry_name| Refind::new().and_then(|mut refind| refind.confirm_boot(&entry_name)));
...
/// # Safety
/// Touches no pointers — rEFInd has no Boot#### entries to register, always succeeds.
#[cfg_attr(feature = "cdylib", unsafe(no_mangle))]
...
fn write_error(err_out: *mut ErrorKind, error: RefindError) {
    if !err_out.is_null() {
